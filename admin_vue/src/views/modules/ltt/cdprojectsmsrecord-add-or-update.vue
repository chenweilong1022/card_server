<template>
  <el-dialog
    ...
    <el-form-item label="缓存的key" prop="key">
      <el-input v-model="dataForm.key" placeholder="缓存的key"></el-input>
    </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        dataRule: {
          key: [
            { required: true, message: '缓存的key不能为空', trigger: 'blur' }
          ],
          code: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/ltt/cdprojectsmsrecord/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.key = data.cdprojectsmsrecord.key
                this.dataForm.code = data.cdprojectsmsrecord.code
                this.dataForm.userId = data.cdprojectsmsrecord.userId
                this.dataForm.projectId = data.cdprojectsmsrecord.projectId
                this.dataForm.deviceId = data.cdprojectsmsrecord.deviceId
                this.dataForm.phone = data.cdprojectsmsrecord.phone
                this.dataForm.iccid = data.cdprojectsmsrecord.iccid
                this.dataForm.deleteFlag = data.cdprojectsmsrecord.deleteFlag
                this.dataForm.createTime = data.cdprojectsmsrecord.createTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ltt/cdprojectsmsrecord/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
